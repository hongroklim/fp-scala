...
class HuffmanSuite extends munit.FunSuite {
  ...
  test("decode and encode with a long tree") {
    new TestTrees {
      assertEquals(decode(t2, encode(t2)("dab".toList)), "dab".toList)
    }
  }

  test("decode and quick encode with a short") {
    new TestTrees {
      assertEquals(decode(t1, quickEncode(t1)("ab".toList)), "ab".toList)
    }
  }

  test("decode and quick encode with a long") {
    new TestTrees {
      assertEquals(decode(t2, quickEncode(t2)("dab".toList)), "dab".toList)
    }
  }
