...
package object barneshut {
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad = {
      if (size <= minimumSize) Leaf(centerX, centerY, size, b+:bodies)
        ...
        val qtrSize = size / 4
        val fork = Fork(
          Empty(centerX - qtrSize, centerY - qtrSize, halfSize),
          Empty(centerX + qtrSize, centerY - qtrSize, halfSize),
          Empty(centerX - qtrSize, centerY + qtrSize, halfSize),
          Empty(centerX + qtrSize, centerY + qtrSize, halfSize)
        )
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface {
    ...
    def +=(b: Body): SectorMatrix = {
      def coordinate(point: Float, minC: Float): Int = {
        val c = (point - minC) / sectorSize
        if (c < 0) 0
        else if (c > sectorPrecision) sectorPrecision
        else c.toInt
      }
