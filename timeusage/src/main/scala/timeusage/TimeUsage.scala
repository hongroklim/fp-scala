...
object TimeUsage extends TimeUsageInterface:
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val P_PREFIX = List("t01", "t03", "t11", "t1801", "t1803")
    val W_PREFIX = List("t05", "t1805")

    val primaries = columnNames.filter(s => P_PREFIX.exists(p => s.startsWith(p)))
    val workings = columnNames.filter(s => W_PREFIX.exists(p => s.startsWith(p)))
    val others = columnNames.filter(s => P_PREFIX.forall(p => !s.startsWith(p)
                                          && W_PREFIX.forall(p => !s.startsWith(p))))

    (primaries.map(col(_)), workings.map(col(_)), others.map(col(_)))
  }
  ...
  def timeUsageSummary(
    ...
    val primaryNeedsProjection: Column = expr("(".concat(primaryNeedsColumns.mkString(" + ")).concat(") / 60")).as("primaryNeeds")
    val workProjection: Column = expr("(".concat(workColumns.mkString(" + ")).concat(") / 60")).as("work")
    val otherProjection: Column = expr("(".concat(otherColumns.mkString(" + ")).concat(") / 60")).as("other")
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] =
    summed.groupByKey(r => (r.working, r.sex, r.age))
          ...
          .withColumn("working", $"key._1")
          .withColumn("sex", $"key._2")
          .withColumn("age", $"key._3")
          .orderBy($"working", $"sex", $"age")
