...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    ...
    val answers = postings.filter(e => e.postingType == 2 && e.parentId.isDefined)
                          .map(e => (e.parentId.get, e))
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] =
    ...
    grouped.map((qid, iter) =>
      (iter.iterator.next._1, answerHighScore(iter.map(_._2).toArray)))
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] =
    ...
    scored.map((q, hs) => (firstLangInTag(q.tags, langs), hs))
          .filter(_._1.isDefined)
          .map((opt, hs) => (opt.get * langSpread, hs))
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] =
    ...
    val median = closestGrouped.mapValues { vs =>
      val common: (LangIndex, Int) = vs.groupBy(_._1)
                                  .map((k, vs) => (k, vs.size))
                                  .toList.sortBy(_._2)(Ordering[Int].reverse)(0) // most common language in the cluster
      val langLabel: String   = langs(common._1/langSpread)
      val langPercent: Double = common._2.toDouble / vs.size * 100 // percent of the questions in the most common language
      val clusterSize: Int    = vs.size

      val vsSorted: List[Int] = vs.map(_._2).toList.sorted
      val mid = clusterSize / 2
      val medianScore: Int    = if (clusterSize%2 == 1) vsSorted(mid)
                                else (vsSorted(mid-1)+vsSorted(mid))/2
